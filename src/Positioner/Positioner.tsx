...
const Positioner = forwardRef<HTMLDivElement, Props>(
  ({ position, hideDelay, showDelay, children }, targetRef) => {
    ...
    useEffect(() => {
      if (typeof targetRef !== 'function' && targetRef?.current) {
        const observerMouseOver$ = fromEvent(targetRef.current, 'mouseenter', {
          capture: true,
    ...
    const update = useCallback(
      (prevHeight = 0, prevWidth = 0) => {
        if (
          !isShown ||
          (typeof targetRef !== 'function' && !targetRef?.current) ||
      ...
      [isShownByTarget, isShownByTooltip, position]
    );
